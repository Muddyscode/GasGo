...
/** Avoid SSR/client drift from zustand persist (session + draft + paid orders). */
export function usePersistHydrated(): boolean {
  ...
  useEffect(() => {
    const done = () => {
      if (
        useOrderDraft.persist.hasHydrated() &&
        useSession.persist.hasHydrated() &&
        useCustomerOrders.persist.hasHydrated()
      ) {
        setHydrated(true);
      }
    };
    ...
    const unsubOrders = useCustomerOrders.persist.onFinishHydration(done);
    return () => {
      unsubDraft();
      unsubSession();
      unsubOrders();
